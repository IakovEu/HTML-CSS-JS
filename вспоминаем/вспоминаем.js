...
// return counter;
// };
// return a;                                     // ~не ставлю вызов функции чтобы сделать это через переменную~
// }

// var increment = CreateCounter();           // ~сделал это чтобы учитывалось лексическое значение (обл. видимости) из замыкания ~
// let c1 = increment();
// let c2 = increment();
// let c3 = increment();
...
// let x = 5; alert( x++ );         // ~При постфиксной записи сначала исходник, при префиксной сразу результат (++x)~

// console.log([ ] + false - null + true);     // ~Пустой массив воспринимается как пустая строка / математические операции со строкой = NaN~

//  let a = [1,2,3];
//  let b = [1,2,3];           
//                                           // ~ Переменные не равны тк это лишь два хранилища с инфомацией~
//  console.log(a == b);
...
// console.log(Math.pow(2, 4));         // ~pow - метод числа для возведения в степень~ 
...
// return x;
// }else {
// return x * pow(x, y - 1);               // ~Рекурсия, функция будет вызывать сама себя до тех пор пока не выполнится верхнее условие~
// }
// }
// console.log(pow(3, 3));
...
// for (let course of Object.values(data)){                // ~Object.values представляет все свойства объекта как массив~
//     if(Array.isArray(course)){                          // ~Array.isArray проверяет на массив выдает true или false~
...
// function getTotalProgressByRecursion(data) {                         // ~Вариант решения через рекурсию~
...
//             subDataArr = getTotalProgressByRecursion(subData);
